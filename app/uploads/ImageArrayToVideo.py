...
class ImageArrayToVideo:
	...
	def convert(self):
		...
		if(os.path.isfile("audio_"+self.original+".aac") ):
			...
			print("--------------################--------------------")
			os.system("ffmpeg -i img_"+self.original+"/output_%d.jpg -i audio_"+self.original+".aac -c:v libx264 -crf 30 -r 25 -pix_fmt yuv420p "+self.videoName+".mkv")
		else:
			print("No Audio")
			print("______________###############-------------")
			os.system("ffmpeg -i img_"+self.original+"/output_%d.jpg -c:v libx264 -crf 30 -r 25 -pix_fmt yuv420p "+self.videoName+".mkv")
